package me.jordanfails.ascendduels.utils.menu

import me.jordanfails.ascendduels.AscendDuels
import me.jordanfails.ascendduels.utils.MinecraftReflection
...
abstract class Menu {
    ...
    internal fun createInventory(player: Player): Inventory {
        ...
        if (placeholder) {
            val placeholder = Button.Companion.placeholder(Material.STAINED_GLASS_PANE, 15.toByte(), " ")
    ...
    fun openMenu(player: Player) {
        if (async) {
            Bukkit.getScheduler().runTask(AscendDuels.instance) {
                try {
    ...
    private fun openCustomInventory(player: Player, inv: Inventory) {
        ...
        if (Bukkit.isPrimaryThread()) {
            ...
            AscendDuels.instance.server.scheduler.runTaskLater(AscendDuels.instance, {
                if (OPEN_CUSTOM_INVENTORY_METHOD_LEGACY != null) {
                    OPEN_CUSTOM_INVENTORY_METHOD_LEGACY.invoke(player, inv, entityPlayer, 0)
                } else {
                    OPEN_CUSTOM_INVENTORY_METHOD!!.invoke(player, inv, entityPlayer, getWindowType(inv.size))
