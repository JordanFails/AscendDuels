        ...
        if (openMenu != null) {
            ...
            if (event.cursor != null && event.cursor.type != Material.AIR && (event.click == ClickType.LEFT || event.click == ClickType.RIGHT || event.click == ClickType.MIDDLE)) {
                if (event.clickedInventory == event.view.topInventory) {
                    ...
                    val itemInserted = when (event.click) {
                        ...
                        ClickType.RIGHT -> {
                            ItemBuilder(event.cursor).amount(1).build()
                        }
                        ClickType.MIDDLE -> {
                            val half = (event.cursor.amount / 2).coerceAtLeast(1)
                            ItemBuilder(event.cursor).amount(half).build()
                        }
                        else -> {
                            event.cursor
                        }
                        ...
                        when (event.click) {
                            ...
                            ClickType.RIGHT -> {
                                if (event.cursor.amount == 1) {
                                    ...
                                    event.cursor = ItemBuilder(event.cursor).amount(event.cursor.amount - 1).build()
                                }
                            }
                            ClickType.MIDDLE -> {
            ...
            if (openMenu.buttons.containsKey(event.slot)) {
                ...
                if (event.isCancelled) {
                    AscendDuels.instance.server.scheduler.scheduleSyncDelayedTask(AscendDuels.instance, {
